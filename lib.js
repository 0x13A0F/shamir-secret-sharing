...
const API_SERVER = "http://localhost:5000/share"
const bits = 2213;
...
function sendToServer(shares, api_key) {
...
function getShareFromServer(api_key, hash) {
...
function split(secret, n, k) {
    if (Number.isInteger(secret))
        throw new TypeError('The secret must be a number');
    if (secret.substring(0, 2) !== '0x')
        throw new TypeError('The secret must be a hexadecimal string starting with 0x');
